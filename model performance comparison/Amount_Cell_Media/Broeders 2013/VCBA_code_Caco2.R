vcba.che.list <- read.csv("VCBA.che.list_Caco2.csv")
...
for(i in 1:2){
  ...
  faq<-0.87 #Caco2 cell line aqueous fraction
  fL<-0.03 #Caco2 cell line lipidic fraction
  fP<-0.1 #Caco2 cell line proteic fraction
  #densities in g/L
  ...
  coreModel<-function(ci,cell_i,numCells,nec,kt) {
    ...
    numCells<-26000 ##Initial cell number
    ncells<-numCells
    ...
    for (kl in 1:kl_fin) {
      ...
      if(ncells<1) {
        ...
        mcells<-5E-9*N[1]+5E-9*N[2]+5E-9*N[3]+5E-9*N[4] # recalculate the total mass of cells
        Vcells<-5E-15*N[1]+5E-15*N[2]+5E-15*N[3]+5E-15*N[4] # recalculate the total volume of cells 
        Wcell<-mcells/ncells
      ...
      valdead<-N[1]*(1-(PS[1]+GS[1]))*5E-9+N[2]*(1-(PS[2]+GS[2]))*5E-9+N[3]*(1-(PS[3]+GS[3]))*5E-9+N[4]*(1-(PS[4]+GS[4]))*5E-9
  ...
  vcba.che.list[i, 8:14] <- 
    coreModel(ci = vcba.che.list$CNOM_M[i], cell_i = 0, numCells = 26000, nec = 1, kt = 0) #ci unit: M
  
  print(i)
}

V_cell <- 5e-12 #L (5e-15 m3)

vcba.che.list$Cell_nmole <- vcba.che.list$x3*V_cell*vcba.che.list$ncells*(1E+6)

write.csv(vcba.che.list, "vcba.che.list_results_Caco2.csv")
