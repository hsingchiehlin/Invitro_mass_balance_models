...
for(i in 1:3){
  ...
  faq<-0.8735 #Caco2 cell line aqueous fraction
  fL<-0.0319 #Caco2 cell line lipidic fraction
  fP<-0.0946 #Caco2 cell line proteic fraction
  #densities in g/L
  ...
  coreModel<-function(ci,cell_i,numCells,nec,kt) {
    ...
    numCells<-100000 ##Initial cell number
    ncells<-numCells
    N<-c(0,100,0,0)*ncells/100 #fraction of cells in each cell cycle phase (50.7,19.2,18.1798,11.9202)
    N<-array(N,dim=c(4))
    mcells_old<-5E-9*N[1]+5E-9*N[2]+5E-9*N[3]+5E-9*N[4] #Cell Mass per cell cycle phase In g
    mcells<-mcells_old
    Vcells<-5E-15*N[1]+5E-15*N[2]+5E-15*N[3]+5E-15*N[4] #Cell volume per cell cycle phase in m^3
    Lt<-L0
    St<-S0
    weight_change<-0
    chemdead<-0
    ...
    for (kl in 1:kl_fin) {
      ...
      valdead<-N[1]*(1-(PS[1]+GS[1]))*3.4E-9+N[2]*(1-(PS[2]+GS[2]))*3.4E-9+N[3]*(1-(PS[3]+GS[3]))*3.4E-9+N[4]*(1-(PS[4]+GS[4]))*3.4E-9
  ...
  vcba.che.list[i, 8:14] <- 
    coreModel(ci = vcba.che.list$CNOM_M[i], cell_i = 0, numCells = 100000, nec = 1, kt = 0) #ci unit: M
  
  print(i)
}

V_cell <- 3.4E-12 #L (5e-15 m3)
