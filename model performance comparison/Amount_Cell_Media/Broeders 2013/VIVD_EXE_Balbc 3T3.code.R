...
VIVD.Che.list <- read.csv("VIVD.Che.para_Balbc 3T3.csv")
...
for(i in 1){
  ...
  Q_total <- paste("Q_total = ", 2000*0.000001*0.000001*VIVD.Che.list$CNOM_uM[i], ";\n", sep="")
    ...
    file.name <- paste("VIVD_dynamic_", "3T3", ".in", sep="")
...
Celldiameter = 16.6;     # (microm) 8.3*2
ncell        = 15000;\n\n')
...
f_NL    = 0.024;  # fraction (v/v) neutral lipid = Storage lipids FROM IVMBM
f_NLrbc = 0.0017;  # ~        in red blood cells

f_NP    = 0.118;  # fraction (v/v) neutral phospholipid = Membrane lipids FROM IVMBM
f_NPrbc = 0.0029;  # ~        in red blood cells

f_IW    = 0.614;   # fration (v/v) intracellular water
...
f_serum       = 0.05;      # fraction (v/v) of serum in total media
pH_serum      = 7.4;       # FBS pH
f_FBS_albumin = 0.04677;   # fraction (v/v) of albumin in FBS
f_FBS_NL      = 0.0016;   # fraction (v/v) of neutral lipid in FBS 
...
write.csv(VIVD.Che.list, "VIVD.drug.sim.results_Balbc 3T3.csv")
