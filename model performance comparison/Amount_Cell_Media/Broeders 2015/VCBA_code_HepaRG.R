vcba.che.list <- read.csv("VCBA.che.list_HepaRG.csv")
...
for(i in 1:2){
  ...
  V<-0.000002 #Volume of medium in the well 0.0003 (m3)
  ##Equations
  ...
  S0<-0.0234*0.4 #protein content for 5 % serum -> 10% *2
  L0<-0.08*0.4 #lipid content for 5% serum
  ...
  coreModel<-function(ci,cell_i,numCells,nec,kt) {
    ...
    numCells<-250000 ##Initial cell number
    ncells<-numCells
    N<-c(0,100,0,0)*ncells/100 #fraction of cells in each cell cycle phase (50.7,19.2,18.1798,11.9202)
    N<-array(N,dim=c(4))
    mcells_old<-3.4E-9*N[1]+3.4E-9*N[2]+3.4E-9*N[3]+3.4E-9*N[4] #Cell Mass per cell cycle phase In g
    mcells<-mcells_old
    Vcells<-3.4E-15*N[1]+3.4E-15*N[2]+3.4E-15*N[3]+3.4E-15*N[4] #Cell volume per cell cycle phase in m^3
    Lt<-L0
    St<-S0
    weight_change<-0
    chemdead<-0
    ...
    for (kl in 1:kl_fin) {
      ...
      if(ncells<1) {
        ...
        mcells<-3.4E-9*N[1]+3.4E-9*N[2]+3.4E-9*N[3]+3.4E-9*N[4] # recalculate the total mass of cells
        Vcells<-3.4E-15*N[1]+3.4E-15*N[2]+3.4E-15*N[3]+3.4E-15*N[4] # recalculate the total volume of cells 
        Wcell<-mcells/ncells
      ...
      valdead<-N[1]*(1-(PS[1]+GS[1]))*3.4E-9+N[2]*(1-(PS[2]+GS[2]))*3.40E-9+N[3]*(1-(PS[3]+GS[3]))*3.4E-9+N[4]*(1-(PS[4]+GS[4]))*3.4E-9
  ...
  vcba.che.list[i, 8:14] <- 
    coreModel(ci = vcba.che.list$CNOM_M[i], cell_i = 0, numCells = 250000, nec = 1, kt = 0) #ci unit: M
  
  print(i)
}

V_cell <- 3.4e-12 #L (3.4e-15 m3)

vcba.che.list$Cell_nmole <- vcba.che.list$x3*V_cell*vcba.che.list$ncells*(1E+6)

write.csv(vcba.che.list, "vcba.che.list_results_HepaRG.csv")
