vcba.che.list <- read.csv("VCBA.che.list_PRH.csv")
...
for(i in 1:2){
  ...
  diaTop<-0.0354 #Diameter of the top of the well
  diaBot<-0.0348 #Dimater of the bottom of the well
  Depth<-0.0174#Depth of the well
  V<-0.0000015 #Volume of medium in the well 0.0003 (m3)
  ##Equations
  ...
  faq<-0.8735 #3T3 cell line aqueous fraction
  fL<-0.0319 #3T3 cell line lipidic fraction
  fP<-0.0946 #3T3 cell line proteic fraction
  #densities in g/L
  ...
  S0<-0.006623484 #protein content (mol/m3) 0.44 (g/L)/66430.3 (g/mol)*1000 (L->m3)
  L0<-0.08*0 #lipid content for 5% serum
  ...
  coreModel<-function(ci,cell_i,numCells,nec,kt) {
    ...
    numCells<-1500000 ##Initial cell number
    ncells<-numCells
    N<-c(0,100,0,0)*ncells/100 #fraction of cells in each cell cycle phase (50.7,19.2,18.1798,11.9202)
    N<-array(N,dim=c(4))
    mcells_old<-6.9E-9*N[1]+6.9E-9*N[2]+6.9E-9*N[3]+6.9E-9*N[4] #Cell Mass per cell cycle phase In g
    mcells<-mcells_old
    Vcells<-6.9E-15*N[1]+6.9E-15*N[2]+6.9E-15*N[3]+6.9E-15*N[4] #Cell volume per cell cycle phase in m^3
    Lt<-L0
    St<-S0
    weight_change<-0
    chemdead<-0
    ...
    for (kl in 1:kl_fin) {
      ...
      if(ncells<1) {
        ...
        mcells<-6.9E-9*N[1]+6.9E-9*N[2]+6.9E-9*N[3]+6.9E-9*N[4] # recalculate the total mass of cells
        Vcells<-6.9E-15*N[1]+6.9E-15*N[2]+6.9E-15*N[3]+6.9E-15*N[4] # recalculate the total volume of cells 
        Wcell<-mcells/ncells
      ...
      valdead<-N[1]*(1-(PS[1]+GS[1]))*6.9E-9+N[2]*(1-(PS[2]+GS[2]))*6.90E-9+N[3]*(1-(PS[3]+GS[3]))*6.9E-9+N[4]*(1-(PS[4]+GS[4]))*6.9E-9
  ...
  vcba.che.list[i, 8:14] <- 
    coreModel(ci = vcba.che.list$CNOM_M[i], cell_i = 0, numCells = 1500000, nec = 1, kt = 0) #ci unit: M
  
  print(i)
}

V_cell <- 6.9e-12 #L (6.9e-15 m3)

vcba.che.list$Cell_nmole <- vcba.che.list$x3*V_cell*vcba.che.list$ncells*(1E+6)

write.csv(vcba.che.list, "vcba.che.list_results_PRH.csv")
