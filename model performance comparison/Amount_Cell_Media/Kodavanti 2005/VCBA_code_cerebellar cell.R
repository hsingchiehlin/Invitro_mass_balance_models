vcba.che.list <- read.csv("VCBA.che.list_cerebellar cell.csv")
...
for(i in 1:12){
  ...
  V<-1.0E-6 #Volume of medium in the well 0.0003 (m3)
  ##Equations
  ...
  faq<-0.018 #3T3 cell line aqueous fraction
  fL<-0.953 #3T3 cell line lipidic fraction
  fP<-0.029 #3T3 cell line proteic fraction
  #densities in g/L
  ...
  coreModel<-function(ci,cell_i,numCells,nec,kt) {
    ...
    for (kl in 1:kl_fin) {
      ...
      if(ncells<1) {
        ...
        mcells<-2.57E-9*N[1]+2.57E-9*N[2]+2.57E-9*N[3]+2.57E-9*N[4] # recalculate the total mass of cells
        Vcells<-2.57E-15*N[1]+2.57E-15*N[2]+2.57E-15*N[3]+2.57E-15*N[4] # recalculate the total volume of cells 
        Wcell<-mcells/ncells
      ...
      valdead<-N[1]*(1-(PS[1]+GS[1]))*2.57E-9+N[2]*(1-(PS[2]+GS[2]))*2.570E-9+N[3]*(1-(PS[3]+GS[3]))*2.57E-9+N[4]*(1-(PS[4]+GS[4]))*2.57E-9
...
V_cell <- 2.57e-12 #L (2.4e-15 m3)

vcba.che.list$Cell_nmole <- vcba.che.list$x3*V_cell*vcba.che.list$ncells*(1E+6)

write.csv(vcba.che.list, "vcba.che.list_results_cerebellar cell.csv")
