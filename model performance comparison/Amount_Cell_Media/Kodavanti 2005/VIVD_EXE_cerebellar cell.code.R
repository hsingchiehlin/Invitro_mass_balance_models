...
VIVD.Che.list <- read.csv("VIVD.Che.para_cerebellar cell.csv")
...
for(i in 1:12){
  ...
  Q_total <- paste("Q_total = ", 1000*0.000001*0.000001*VIVD.Che.list$CNOM_uM[i], ";\n", sep="")
  
    
    file.name <- paste("VIVD_dynamic_", "cerebellarcell.", i, ".in", sep="")
...
f_NL    = 0;  # fraction (v/v) neutral lipid = Storage lipids FROM IVMBM
f_NLrbc = 0.0017;  # ~        in red blood cells

f_NP    = 0.953;  # fraction (v/v) neutral phospholipid = Membrane lipids FROM IVMBM
f_NPrbc = 0.0029;  # ~        in red blood cells

f_IW    = 0.018;   # fration (v/v) intracellular water
...
End.\n')      
    ...
    VIVD.Che.list[i, c(12:29)] <- out[31, ] # put %abs into the "mc_sim_results"
...
write.csv(VIVD.Che.list, "VIVD.drug.sim.results_cerebellar cell.csv")
