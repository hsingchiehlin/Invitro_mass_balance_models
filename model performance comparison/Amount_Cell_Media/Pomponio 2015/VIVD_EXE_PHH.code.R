...
VIVD.Che.list <- read.csv("VIVD.Che.para_PHH.csv")
...
for(i in 1:4){
  ...
  Q_total <- paste("Q_total = ", 2000*0.000001*0.000001*VIVD.Che.list$CNOM_uM[i], ";\n", sep="")
    ...
    file.name <- paste("VIVD_dynamic_", "PHH.", i, ".in", sep="")
...
f_NP    = 0.002233;  # fraction (v/v) neutral phospholipid = Membrane lipids FROM IVMBM
...
write.csv(VIVD.Che.list, "VIVD.drug.sim.results_PHH.csv")
