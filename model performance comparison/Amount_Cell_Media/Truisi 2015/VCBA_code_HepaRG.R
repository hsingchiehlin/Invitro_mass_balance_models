...
for(i in 1:2){
  ...
  coreModel<-function(ci,cell_i,numCells,nec,kt) {
    ...
    numCells<-2000000 ##Initial cell number
    ncells<-numCells
    ...
    mcells<-mcells_old
    Vcells<-1.73E-15*N[1]+3.4E-15*N[2]+3.4E-15*N[3]+3.4E-15*N[4] #Cell volume per cell cycle phase in m^3
    Lt<-L0
    St<-S0
    weight_change<-0
    chemdead<-0
  ...
  vcba.che.list[i, 8:14] <- 
    coreModel(ci = vcba.che.list$CNOM_M[i], cell_i = 0, numCells = 2E+6, nec = 1, kt = 0) #ci unit: M
