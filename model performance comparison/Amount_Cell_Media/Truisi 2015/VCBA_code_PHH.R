vcba.che.list <- read.csv("VCBA.che.list_PHH.csv")
...
for(i in 1:4){
  ...
  S0<-0.0234*0 #protein content for 5 % serum -> 10% *2
  ...
  coreModel<-function(ci,cell_i,numCells,nec,kt) {
    ...
    numCells<-2E+6 ##Initial cell number
    ncells<-numCells
...
write.csv(vcba.che.list, "vcba.che.list_results_PHH.csv")
