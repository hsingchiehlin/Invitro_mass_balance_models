vcba.che.list <- read.csv("VCBA.che.list_ARE.csv")
...
for(i in 1:8){
  ...
  diaBot<-0.0064 #Dimater of the bottom of the well
  Depth<-10.76*0.001 #Depth of the well
  V<-2.2E-7 #Volume of medium in the well 0.0003
  ##Equations
  ...
  faq<-0.944 #3T3 cell line aqueous fraction
  fL<-0.005 #3T3 cell line lipidic fraction
  fP<-0.051 #3T3 cell line proteic fraction
  #densities in g/L
  ...
  S0<-0.0234*2 #protein content for 5 % serum -> 10% *2
  L0<-0.08*2 #lipid content for 5% serum
  ...
  coreModel<-function(ci,cell_i,numCells,nec,kt) {
    ...
    numCells<-20000 ##Initial cell number
    ncells<-numCells
  ...
  vcba.che.list[i, 8:14] <- 
    coreModel(ci = vcba.che.list$CNOM_M[i], cell_i = 0, numCells = 20000, nec = 1, kt = 0) #ci unit: M
  
  print(i)
}

vcba.che.list$F_free <- vcba.che.list$Cdcomp/vcba.che.list$x1

write.csv(vcba.che.list, "vcba.che.list_hsu_ARE.csv")




vcba.che.list <- read.csv("VCBA.che.list_PPAR.csv")
...
for(i in 1:8){
  ...
  logkow<-vcba.che.list$logkow[i] #logarithm of partition octanol-water
  #mass fractions in % weight
  faq<-0.91 #3T3 cell line aqueous fraction
  fL<-0.01 #3T3 cell line lipidic fraction
  fP<-0.08 #3T3 cell line proteic fraction
  #densities in g/L
  rhoaq<-1000 #aqueous phase density
  rhoL<-900 #lipid phase density
  rhoP<-1350 #proteic phase density
  #Lipid and protein in medium serum. Mind the % of serum supplementing the medium
  S0<-(0.0234/5)*2 #protein content for 5 % serum -> 10% *2
  L0<-(0.08/5)*2 #lipid content for 5% serum
  ##Equations for partition
  #plastic well partition
  Kp<-10^(0.97*logkow-6.94)
  #serum well partition
...
write.csv(vcba.che.list, "vcba.che.list_hsu_PPAR.csv")
