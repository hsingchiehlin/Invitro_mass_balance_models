...
VIVD.Che.list <- read.csv("VIVD.Che.para.drug_hus_ARE.csv")
...
for(i in 1:8){
  ...
  Q_total <- paste("Q_total = ", 220*0.000001*0.000001*VIVD.Che.list$CNOM_uM[i], ";\n", sep="")
...
f_NL    = 0.00046;  # fraction (v/v) neutral lipid 
f_NLrbc = 0.0017;  # ~        in red blood cells

f_NP    = 0.00414;  # fraction (v/v) neutral phospholipid
f_NPrbc = 0.0029;  # ~        in red blood cells

f_IW    = 0.944;   # fration (v/v) intracellular water
...
pH_medium     = 7.2;       # culture medium pH
...
End.\n')      
    ...
    VIVD.Che.list[i, c(12:29)] <- out[25, ] # put %abs into the "mc_sim_results"
...
write.csv(VIVD.Che.list, "VIVD.drug.sim.results.hsu_ARE.csv")



VIVD.Che.list <- read.csv("VIVD.Che.para.drug_hus_PPAR.csv")
for(i in 1:8){
...
f_NL    = 0.001;  # fraction (v/v) neutral lipid 
f_NLrbc = 0.0017;  # ~        in red blood cells

f_NP    = 0.009;  # fraction (v/v) neutral phospholipid
f_NPrbc = 0.0029;  # ~        in red blood cells

f_IW    = 0.91;   # fration (v/v) intracellular water
f_IWrbc = 0.666;   # ~       for red blood cells
...
write.csv(VIVD.Che.list, "VIVD.drug.sim.results.hsu_PPAR.csv")



MassBalan.results <- read.csv("VIVD.drug.sim.results.csv")
...
# courtney's results plotting
MassBalan.results <- read.csv("VIVD.drug.sim.results.allmodels.csv")
MassBalan.results$Model <- factor(MassBalan.results$Model)

tiff(width = 17, height = 7.5, "Figure.drugsimresults2.tiff", units = "in", res=300)
ggplot(MassBalan.results, aes(x = Cnominal, y = Cfree, group = Model))+
  geom_point(aes(color = Model, size = Model, shape = Model), alpha=0.6, size=2) +
  geom_abline(intercept = 0, slope = 1, linetype=2)+
  facet_grid(Medium ~ Compound)+
  xlab("Cnominal (uM)") +
  ylab("Cfree (uM)")+
  scale_color_manual(values = c("#66a841", "#268AFF", "#f7470e")) + 
  scale_x_log10(lim = c(1E-1, 1E+3),
                breaks = trans_breaks("log10", function(x) 10^x, n = 4),
                labels = trans_format("log10", math_format(10^.x))) +
  scale_y_log10(lim = c(1E-1, 1E+3),
                breaks = trans_breaks("log10", function(x) 10^x, n = 4),
                labels = trans_format("log10", math_format(10^.x))) +
  theme_bw() +
  theme(text=element_text(family="sans", face="plain", color="#000000", size=10, hjust=0.5, vjust=0.5), 
