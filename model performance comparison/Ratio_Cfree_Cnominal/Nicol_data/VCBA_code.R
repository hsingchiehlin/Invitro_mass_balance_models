...
for(i in c(1:2,4:30)){
  ...
  diaTop<-0.006 #Diameter of the top of the well #----revise 
  diaBot<-0.0059 #Dimater of the bottom of the well #----revise 
  Depth<-0.0215#Depth of the well #----revise  
  V<-3.0E-7 #Volume of medium in the well 0.0003 (m3) #----revise 
  ##Equations
  H<-diaTop*Depth/(diaTop-diaBot)
  h<-H-Depth
  #Total Volume well:
  vVessel<-6.0E-7 #----revise 
  ...
  S0<-0.0234*(0.096660808/0.1*2) #protein content for 5 % serum -> 10% *2
  L0<-0.08*(0.096660808/0.1*2) #lipid content for 5% serum
  ...
  vcba.che.list[i, 7:13] <- 
    coreModel(ci = 1e-4, cell_i = 0, numCells = 1e-40, nec = 1, kt = 0)
  
  print(i)
}

vcba.che.list$F_free <- vcba.che.list$Cdcomp/vcba.che.list$x1
write.csv(vcba.che.list, "vcba.che.list_HepG2.csv")
...
vcba.che.list$F_free <- vcba.che.list$Cdcomp/vcba.che.list$x1
write.csv(vcba.che.list, "vcba.che.list_HepRG.csv")
...
for(i in c(1:2,4:30)){
  ...
  rhoP<-1350 #proteic phase density
  #Lipid and protein in medium serum. Mind the % of serum supplementing the medium
  S0<-0.0234*(0.100795756/0.1*2) #protein content for 5 % serum -> 10% *2
  L0<-0.08*(0.100795756/0.1*2) #lipid content for 5% serum
  ##Equations for partition
  #plastic well partition
  Kp<-10^(0.97*logkow-6.94)
  #serum well partition
...
vcba.che.list$F_free <- vcba.che.list$Cdcomp/vcba.che.list$x1
write.csv(vcba.che.list, "vcba.che.list_MCF7.csv")