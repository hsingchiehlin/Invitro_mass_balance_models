vcba.che.list <- read.csv("VCBA.che.list_sch.csv")
...
for(i in 1:14){
  ...
  diaTop<-0.0162 #Diameter of the top of the well
  diaBot<-0.0156 #Dimater of the bottom of the well
  ...
  Te<- 19+273.15 #Air temperature from Celsius to K. Experiment temperature constant at 37 C.
  ...
  faq<-0.89 #3T3 cell line aqueous fraction
  fL<-0.05 #3T3 cell line lipidic fraction
  fP<-0.06 #3T3 cell line proteic fraction
  #densities in g/L
  ...
  vcba.che.list[i, 8:14] <- 
    coreModel(ci = vcba.che.list$CNOM_M[i], cell_i = 0, numCells = 120000, nec = 1, kt = 0) #ci unit: M
...
write.csv(vcba.che.list, "vcba.che.list_results_sch.csv")
