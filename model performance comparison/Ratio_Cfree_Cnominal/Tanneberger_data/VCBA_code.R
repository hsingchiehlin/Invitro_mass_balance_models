vcba.che.list <- read.csv("VCBA.che.list_tan.csv")
...
for(i in 1:27){
  ...
  vcba.che.list[i, 8:14] <- 
    coreModel(ci = vcba.che.list$CNOM_M[i], cell_i = 0, numCells = 300000, nec = 1, kt = 0) #ci unit: M
...
write.csv(vcba.che.list, "vcba.che.list_results_tan.csv")
