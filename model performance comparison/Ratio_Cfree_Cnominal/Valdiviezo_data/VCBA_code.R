...
for(i in 1:20){
  ...
  diaTop<-15.2*0.001 #Diameter of the top of the well
  diaBot<-15*0.001 #Dimater of the bottom of the well
  Depth<- 45*0.001 #Depth of the well
  V<-3e-04 #Volume of medium in the well
  ##Equations
  ...
  vcba.che.list[i, 7:13] <- 
    coreModel(ci = 1e-5, cell_i = 0, numCells = 1e-40, nec = 1, kt = 0)
...
vcba.che.list_1uM <- vcba.che.list
vcba.che.list_10uM <- vcba.che.list

write.csv(vcba.che.list_1uM, "vcba.che.list_1uM.csv")
write.csv(vcba.che.list_10uM, "vcba.che.list_10uM.csv")
